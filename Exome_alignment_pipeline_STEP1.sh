...
#end result is multiple (small) .bam files per sample ready for bam processing

#Example usage:
#note options! input dir, output dir and sample ID! pres 
#sh WGS_alignment_pipeline.sh -i /data/kronos/apittman/Exome_Data_Analysis/Whole_Genome_Sequencing_Pipeline/Unaligned -o /data/kronos/apittman/Exome_Data_Analysis/Whole_Genome_Sequencing_Pipeline/a_aligned -l sample_xxx

########################################################################################

novoalign="/data/kronos/NGS_Software/novocraft_v3/novoalign"
novoalignArguments="-c 10 --rOQ --hdrhd 3 -H -k -o Soft -t 320 -F ILM1.8"
...
##removing intermediate files:	
		...
		echo "
			...
			###Remove Intermediate BAM files leaving only final sorted bam files:

			if [ -e ${output}_${n}_sorted.bam ] 
				then
					rm ${output}_${n}.bam
					rm ${output}_${n}.sam

										
					echo 'SAM BAM and intermediate BAM.bai all removed'
				else
...
##now last (but not least) we will automatically submit all jobs to the SGE of KRONOS
	...
	echo "
	=======================================
	STEP THREE - Submitting jobs to Kronos
	=======================================
	"
sleep 1
...
while (( $n <= 7 ))
	...
	n=$(( n+1 ))
		...
		for nID in $myIDs; do
			...
			if [ -e ${iFolder}/${myIDs}/*L00${n}_R1*gz ]; then
				sleep 2
				qsub -pe make 10 -cwd $oFolder/J_script_${nID}_${n}.sh
